...
public interface MotelRepository extends JpaRepository<Motel, Integer> {
    ...
    @Query(value = "select * from motels a where a.account_id = :accountId AND motel_id NOT IN (select motel_id from posts)", nativeQuery = true)
    List<Motel> findPostsByNotMotel(@Param("accountId") Integer accountId);
